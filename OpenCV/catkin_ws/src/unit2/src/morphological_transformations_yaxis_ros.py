...
class Transformations(object):
    ...
    def camera_callback(self,data):
        ...
        erosion = cv2.erode(y_conv,kernel_a,iterations = 1)

        #Define a kernel for the dilation
        kernel_b = np.ones((3,3),np.uint8)
        dilation = cv2.dilate(y_conv,kernel_b,iterations = 1)

        #Define a kernel for the opening
        kernel_c = np.ones((2,2),np.uint8)
        opening = cv2.morphologyEx(y_conv, cv2.MORPH_OPEN, kernel_c)

        #Define a kernel for the closing
        kernel_d = np.ones((2,2),np.uint8)
        closing = cv2.morphologyEx(y_conv, cv2.MORPH_CLOSE, kernel_d)

        cv2.imshow('Original',img)
        cv2.imshow('y_conv',y_conv)
